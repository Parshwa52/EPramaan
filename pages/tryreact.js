...
import { Form, Button, Input, Dropdown,Menu,Radio, Message,Segment,Checkbox,Confirm} from 'semantic-ui-react';
...
export default class Aadhar extends Component{
  constructor(props) {
      super(props);
      this.state = {
      fname: '', date:'',mname: '',lname: '',flat: '', build: '',road: '',land:'', tvc: '', subd: '',defcheck:true,
      dist: '',stat: '',pin:'',gen:'',pass: '', ffname: '',fmname: '', flname: '',mfname: '',mmname: '', mlname: '',fnameerror:false,loading:false,lnameerror:false,mnameerror:false,pinerror:false,passerror:false,ffnameerror:false,flnameerror:false,fmnameerror:false,mfnameerror:false,mlnameerror:false,mmnameerror:false,focus:false
};
  ...
  handleSubmit = async (event) =>{
    ...
    //this.setState({road:});
    this.setState({loading:true,fnameerror:false,lnameerror:false,mnameerror:false,pinerror:false,passerror:false,ffnameerror:false,flnameerror:false,fmnameerror:false,mfnameerror:false,mlnameerror:false,mmnameerror:false,open:false});
  ...
  else if(this.state.pin.length!=6 || isNaN(this.state.pin)===true)
    {
        ...
        {
          this.setState({ffnameerror:true,loading:false});
          this.fmname.focus()
          }
      else if(!this.state.fmname.match(/^[a-zA-Z]+$/))
        {
          this.setState({fmnameerror:true,loading:false});
          this.flname.focus()
            }
            else if(!this.state.flname.match(/^[a-zA-Z]+$/))
              {
                this.setState({flnameerror:true,loading:false});
                this.ffname.focus()
                }
            else if(!this.state.mfname.match(/^[a-zA-Z]+$/))
              {
                this.setState({mfnameerror:true,loading:false});
                this.mlname.focus()
              }
              else if(!this.state.mmname.match(/^[a-zA-Z]+$/))
                {
                  this.setState({mmnameerror:true,loading:false});
                  this.mmname.focus()
                  }
              else if(!this.state.mlname.match(/^[a-zA-Z]+$/))
                {
                  this.setState({mlnameerror:true,loading:false});
                  this.mfname.focus()
                    }
  ...
  render()
  {
    ...
    />
      ...
      color='black'
    />
        <Form error={this.state.fnameerror||this.state.lnameerror||this.state.mnameerror||this.state.pinerror||this.state.passerror||this.state.ffnameerror||this.state.flnameerror||this.state.fmnameerror||this.state.mfnameerror||this.state.mlnameerror||this.state.mmnameerror}>
          <br/>
          <Form.Field>
        ...
        //  labelPosition=""
          ...
          <br/>
          <Form.Field>
          <label color="black" style={{fontSize:'20px'}}> Gender </label>
          </Form.Field>
          ...
          //  labelPosition=""
            ...
            <br/>
            <Form.Field>
            <label color="black" style={{fontSize:'20px'}}> Father's name </label>
            </Form.Field>
            <Form.Group widths='equal'>
          <Form.Field>
          <Input
          label="First Name"
        //  labelPosition=""
          ref={(input) => { this.ffname = input; }}
          value={this.state.ffname}
          onChange={event => this.setState({ffname: event.target.value})}
          />
          </Form.Field>
          <Form.Field>
          <Input
          label="Middle Name"
          //labelPosition=""
            ref={(input) => { this.fmname = input; }}
          value={this.state.fmname}
          onChange={event => this.setState({fmname: event.target.value})}
          />
          </Form.Field>
          <Form.Field>
          <Input
          label="Last Name"
        //  labelPosition=""
          ref={(input) => { this.flname = input; }}
          value={this.state.flname}
          onChange={event => this.setState({flname: event.target.value})}
          />
          </Form.Field>
          </Form.Group>
          ...
          <Form.Field>
          <label color="black" style={{fontSize:'20px'}}> Mother's name </label>
          </Form.Field>
          <Form.Group widths='equal'>
          <Form.Field>
          ...
          label="First Name"
        //  labelPosition=""
          ref={(input) => { this.mfname = input; }}
          value={this.state.mfname}
          onChange={event => this.setState({mfname: event.target.value})}
          />
          </Form.Field>
          <Form.Field>
          <Input
          label="Middle Name"
          //labelPosition=""
            ref={(input) => { this.mmname = input; }}
          value={this.state.mmname}
          onChange={event => this.setState({mmname: event.target.value})}
          />
          </Form.Field>
          <Form.Field>
          <Input
          label="Last Name"
        //  labelPosition=""
          ref={(input) => { this.mlname = input; }}
          value={this.state.mlname}
          onChange={event => this.setState({mlname: event.target.value})}
          />
          </Form.Field>
          </Form.Group>
        ...
        //  labelPosition=""
          ...
          {this.state.passerror
            ?
            <Message color='red' error header="Invalid Content" content="Passport Number should be valid"/>
            :
            null
          }
          <br/>
